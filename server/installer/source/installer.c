...
void patch_kernel() {
    ...
    *(uint8_t *)(kernbase + 0x30D9AA) = 0xEB;
    ...
    // remove all these bullshit checks from ptrace
...
int load_kdebugger() {
    ...
    if (elf_mapped_size(kernelelf, &mapsize)) {
        printf("[PS4MEMAPI] invalid kdebugger elf!\n");
        return 1;
    }
    ...
    if(!kmemory) {
        printf("[PS4MEMAPI] could not allocate memory for kdebugger!\n");
        return 1;
    }

    // load the elf
    if (load_elf(kernelelf, kernelelf_size, kmemory, mapsize, (void **)&payload_entry)) {
        printf("[PS4MEMAPI] could not load kdebugger elf!\n");
        return 1;
    }
...
int load_debugger() {
    ...
    if(!p) {
        printf("[PS4MEMAPI] could not find SceShellCore process!\n");
        return 1;
    }
    ...
    vm_map_unlock(map);
    if(r) {
        printf("[PS4MEMAPI] failed to allocate payload memory!\n");
        return r;
    }
    ...
    if(r) {
        printf("[PS4MEMAPI] failed to write payload!\n");
        return r;
    }
    ...
    if(r) {
        printf("[PS4MEMAPI] failed to create payload thread!\n");
        return r;
    }
...
int runinstaller() {
    ...
    *disable_console_output = 0;
    ...
    printf("[PS4MEMAPI] installer running\n");

    // patch the kernel
    printf("[PS4MEMAPI] patching kernel...\n");
    patch_kernel();

    printf("[PS4MEMAPI] loading kdebugger...\n");

    if(load_kdebugger()) {
        return 1;
    }

    printf("[PS4MEMAPI] loading internal payload...\n");

    if(load_debugger()) {
        return 1;
    }

    printf("[PS4MEMAPI] PS4MEMAPI ready");
